...
This gallery showcases the quiz types available in **rquiz** with different configurations. All quizzes are interactive — try them out! Click "Code" to see the R code behind each quiz.
...
Select all correct answers, then click Submit. The tip and solution buttons provide help.
...
Navigate through questions with Previous/Next. The timer tracks your time and a results page shows your score at the end. With `shuffle = TRUE`, the question order is randomized — click "Try again" on the results page to see a different order each time.
...
When any question has multiple correct answers, all questions use checkboxes.
...
Questions with a single answer and questions with multiple answers can be combined. The quiz auto-detects MC mode when any question has multiple correct answers.
...
Use `<pre>` tags for monospace code formatting. Use `<br>` for line breaks within `<pre>` blocks.
...
All UI text (buttons, messages, tips) adapts automatically with `language = "de"`:
...
Create a theme once and apply it to any quiz type with `rquizTheme()`:
...
Navigation buttons, solution area, and answers can all be themed individually:
...
Quizzes adapt responsively to smaller widths. Buttons wrap to the next line when space is limited.
...
Use `scroll = TRUE` for fixed-height containers — ideal for HTML slide presentations:
...
```{=html}
<script>
// Repeatedly scroll to top until page is fully settled
(function() {
  var attempts = 0;
  var maxAttempts = 20;
